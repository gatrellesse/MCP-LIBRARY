...
@mcp.tool()
def add_book(title: str, author: str, genre: str, synopsis: str):
    """Add a new book to the shared library."""
    with BookQdrant() as qdrant:
        book_id = qdrant.add_to_library(title=title, author=author, genre=genre, synopsis=synopsis)
    if book_id is None:
...
def delete_book(title: str):
    ...
    with BookQdrant() as qdrant:
        found = qdrant.delete_from_library(title=title)
    if not found:
...
def get_book(title: str):
    ...
    with BookQdrant() as qdrant:
        book = qdrant.get_book(title=title)
    if book is None:
        raise ValueError(f"Book '{title}' not found in the library.")
    return book
...
@mcp.tool()
def search_library(query: str, author: str | None = None, genre: str | None = None):
    """Semantic search across the entire library."""
    with BookQdrant() as qdrant:
        return qdrant.search_library(query=query, author=author, genre=genre)
