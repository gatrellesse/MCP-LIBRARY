...
class BookQdrant:
    ...
    def _resolve_library_id(self, title: str) -> str | None:
    ...
    def _resolve_bookmark_id(self, user_id: str, title: str) -> str | None:
        results, _ = self.client.scroll(
            collection_name=BOOKMARKS,
            scroll_filter=Filter(must=[
    ...
    def add_to_library(self, title: str, author: str, genre: str, synopsis: str) -> str | None:
    ...
    def get_book(self, title: str) -> dict | None:
        """Retrieve a book from the library by title."""
        book_id = self._resolve_library_id(title)
        if book_id is None:
            return None
    ...
    def delete_from_library(self, title: str) -> bool:
        ...
        if book_id is None:
            return False
        self.client.delete(collection_name=LIBRARY, points_selector=[book_id])
        return True

    def update_text_embedding(self, title: str, text: str) -> bool:
    ...
    def add_bookmark(self, user_id: str, title: str, status: str = "wanting") -> bool:
    ...
    def update_status(self, user_id: str, title: str, status: str) -> bool:
        """Update the reading status of a user's bookmark."""
        bookmark_id = self._resolve_bookmark_id(user_id, title)
    ...
    def delete_bookmark(self, user_id: str, title: str) -> bool:
        """Remove a user's bookmark."""
        bookmark_id = self._resolve_bookmark_id(user_id, title)
        if bookmark_id is None:
            return False
        self.client.delete(collection_name=BOOKMARKS, points_selector=[bookmark_id])
        return True

    def list_user_books(self, user_id: str, status: str | None = None) -> list[dict]:
    ...
    def search_user_books(
        ...
        bookmarks, _ = self.client.scroll(
            collection_name=BOOKMARKS,
            scroll_filter=Filter(must=must_conditions),
            with_payload=True,
            limit=1000,
